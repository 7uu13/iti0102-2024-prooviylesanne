def parse_followers(call_log: str) -> dict:
    """
    ...
    for line in call_log.split(","):
        names = line.split(":")
        l, r = 0, 1

        while r <= len(names) - 1:
            if names[l] not in res_dict:
                res_dict[names[l]] = [names[r]]
            else:
                if names[r] not in res_dict[names[l]]:
                    res_dict[names[l]].append(names[r])
            l += 1
            r += 1
    return res_dict
